...
class MemoryTool(BaseTool):
    metadata = ToolMetadata(
        ...
        keywords=("remember ", "recall ", "search memory", "what do you remember", "forget memory", "summarize memory"),
    ...
    def execute(self, user_text: str, *, confirmed: bool = False) -> ToolResult:
        ...
        if lowered.startswith(("recall ", "search memory")):
            ...
            records = self.memory_manager.search(query)
            if not records:
                return ToolResult(True, "No memory found.", {"matches": []})
